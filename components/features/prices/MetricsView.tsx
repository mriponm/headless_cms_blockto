...
const STATS = [
  { label: "Bitcoin", value: "$84,231", sub: "+$1,972 (2.4%) today", up: true, hero: true },
      ...
      {rows.map((r) => (
        <div key={r.sym} className="mtr-coin-row">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="mtr-nm">{r.name}</div>
              <div className="mtr-sy">{r.sym}</div>
            </div>
          </div>
...
export default function MetricsView() {
  return (
    <div className="mtr-root">
      ...
      {/* ── Key stats ─────────────────────────── */}
      <div className="mtr-stats-grid">
        {STATS.map((s) => (
          <div key={s.label} className={`mtr-stat glass${s.hero ? " mtr-stat-hero" : ""}`}>
            <div className={`mtr-stat-label${s.hero ? " mtr-stat-label-hero" : ""}`}>{s.label}</div>
            <div className={`mtr-stat-val${s.hero ? " mtr-stat-val-hero" : ""}`}>{s.value}</div>
      ...
      <div className="glass-strong mtr-card">
        <div className="mtr-card-head">
          ...
          <span className="mtr-card-sub">Market sentiment</span>
        </div>
        <div className="mtr-card-body mtr-fg-wrap">
          {/* Gauge */}
          <div className="mtr-gauge-wrap">
          ...
          <div className="flex-1">
            {[
              ...
              { k: "Last year",  v: "74 — Greed",       c: "#00d47b" },
            ].map((row) => (
              <div key={row.k} className="mtr-fg-row">
                <span className="mtr-fg-key">{row.k}</span>
                <span style={{ color: row.c, fontSize: 12, fontWeight: 600 }}>{row.v}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="glass-strong mtr-card">
        ...
        <div className="mtr-card-body mtr-hlv-wrap">
          <div className="mtr-hlv-ring-wrap" style={{ position: "relative", width: 74, height: 74, flexShrink: 0 }}>
            <svg viewBox="0 0 74 74" width="74" height="74" style={{ transform: "rotate(-90deg)" }}>
              <defs>
          ...
          <div>
            ...
            <div className="mtr-hlv-sub">Block reward: 3.125 → 1.5625 BTC</div>
      ...
      <div className="glass-strong mtr-card">
        ...
        <div className="mtr-card-body">
          ...
          <div className="mtr-as-lbl"><span>Bitcoin season</span><span>Altcoin season</span></div>
          <p className="mtr-as-note">32% of the top 50 altcoins outperformed BTC over the last 90 days.</p>
        </div>
      </div>
          ...
          {STABLES.map((s) => (
            <div key={s.name} className="mtr-sc-row">
              ...
              <div className="mtr-sc-bar-wrap">
                <div className="mtr-sc-bar-fill" style={{ width: `${s.pct}%`, background: s.color }} />
              </div>
